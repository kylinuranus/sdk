...
//默认control参数
#define kPageControlW 8
#define kPageControlH 2
#define kControlSpace 3

@interface SOSEllipsePageControl ()


@end

@implementation SOSEllipsePageControl
...
-(void)initialize{
    ...
    _numberOfPages=0;
    _currentPage=0;
    _controlWidth    = kPageControlW;
    _controlHeight  = kPageControlH;
    _controlSpacing = kControlSpace;
//    _otherColor=[UIColor colorWithHexString:@"#C3CEEC"];
//    _currentColor=[UIColor colorWithHexString:@"#6896ED"];
...
-(void)setcontrolWidth:(NSInteger)controlWidth{
    if(controlWidth!=_controlWidth){
...
-(void)createPointView{
    ...
    CGFloat startX=0;
    CGFloat startY=0;
    CGFloat mainWidth=_numberOfPages*(_controlWidth+_controlSpacing);
    if(self.frame.size.width<mainWidth){
        startX=0;
    }else{
        startX=(self.frame.size.width-mainWidth)/2;
    }
    if(self.frame.size.height<_controlWidth){
        startY=0;
    }else{
    ...
    for (int page=0; page<_numberOfPages; page++) {
        if(page==_currentPage){
            UIView *currPointView=[[UIView alloc]initWithFrame:CGRectMake(startX, startY, _controlWidth*2, _controlHeight)];
            ...
            UIView *otherPointView=[[UIView alloc]initWithFrame:CGRectMake(startX, startY, (_controlWidth), _controlHeight)];
            otherPointView.backgroundColor=_otherColor;
            otherPointView.layer.cornerRadius = _controlHeight/2;
...
-(void)exchangeCurrentView:(NSInteger)old new:(NSInteger)new
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if(new<old)
            lx+=self->_controlWidth;
        oldSelect.frame=CGRectMake(lx, mpSelect.origin.y, self->_controlWidth, self->_controlHeight);
        
        CGFloat mx=newTemp.origin.x;
        if(new>old)
            mx-=self->_controlWidth;
        newSeltect.frame=CGRectMake(mx, newTemp.origin.y, self->_controlWidth*2, self->_controlHeight);
        ...
        if(new-old>1)
        {
            for(NSInteger t=old+1;t<new;t++)
            {
                ...
                ms.frame=CGRectMake(ms.frame.origin.x-self->_controlWidth, ms.frame.origin.y, self->_controlWidth, self->_controlHeight);
            }
        }
        // 右边选中到左边的时候 越过点击
        if(new-old<-1)
