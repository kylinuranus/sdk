...
- (void)viewDidLoad    {
    ...
    [self.collectionView setContentInset:UIEdgeInsetsMake(weekdayHeaderHeight, 10, 0, 10)];
}
...
//    NSDateComponents *firstOfMonthsComponents = [self.calendar components:kCalendarUnitYMD fromDate:firstOfMonth];
    ...
    cell.enabled = [self collectionView:collectionView shouldSelectItemAtIndexPath:indexPath];

    //If the current Date is not enabled, or if the delegate explicitely specify custom colors
    if (cell.enabled && isCustomDate) {
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath    {
    if (kind == UICollectionElementKindSectionHeader) {
        ...
        headerView.titleLabel.text = [NSString stringWithFormat:@"%@", @([self firstOfMonthForSection:indexPath.section].month)];
