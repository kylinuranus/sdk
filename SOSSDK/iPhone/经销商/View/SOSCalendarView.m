...
@implementation SOSCalendarView
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        if (ISIPAD) {
            [self.prevButton setTitle:@"上一月" forState:0];
            [nextButton setTitle:@"下一月" forState:0];
        }
...
- (void)layoutSubviews {
    ...
    if (ISIPAD) {
        self.prevButton.frame = CGRectMake(BUTTON_MARGIN, BUTTON_MARGIN, 48 *2, 38);
        self.nextButton.frame = CGRectMake(self.bounds.size.width - 48 - BUTTON_MARGIN - 40, BUTTON_MARGIN, 48 * 2, 38);
    }
...
- (void)setButtonColor:(UIColor *)color {
    [self.prevButton setImage:[SOSCalendarView imageNamed:@"left_arrow.png" withColor:color] forState:UIControlStateNormal];
    [self.nextButton setImage:[SOSCalendarView imageNamed:@"right_arrow.png" withColor:color] forState:UIControlStateNormal];
}
