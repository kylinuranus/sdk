...
#define HDContentHeight ([UIScreen mainScreen].bounds.size.height - [UIApplication sharedApplication].statusBarFrame.size.height)
...
- (instancetype)init {
    self = [super initWithFrame:CGRectMake(0, [UIApplication sharedApplication].statusBarFrame.size.height, [UIScreen mainScreen].bounds.size.width, HDContentHeight)];
    if (self) {
 ...
 */
+ (void)changeVisible {
    if ([HDWindowLogger defaultWindowLogger].visible) {
        [self defaultWindowLogger].visible = NO;
        [self hideLogWindow];
        [self defaultWindowLogger].mFloatWindow.hidden = YES;
    }else {
        [self defaultWindowLogger].visible = YES;
        [self hideLogWindow];
    }
}
...
- (void)p_createUI {
    ...
    [self.mTableView setFrame:CGRectMake(0, 40, [UIScreen mainScreen].bounds.size.width, HDContentHeight - 80)];
    ...
    [self.mSearchBar setFrame:CGRectMake(0, HDContentHeight - 40, [UIScreen mainScreen].bounds.size.width, 40)];
}
...
- (void)p_share {
    ...
    NSString *fileName = [NSString stringWithFormat:@"SOSiOSLog.txt"];// 注意不是NSData!
...
- (UITableView *)mTableView {
    if (!_mTableView) {
        ...
        _mTableView.estimatedRowHeight = 100.0f;
...
//        _mTableView.estimatedRowHeight =
...
- (UIButton *)mCleanButton {
    if (!_mCleanButton) {
        ...
        [_mCleanButton setTitle:
                       NSLocalizedStringFromTable(@"清除Log", @"logLocalizable", nil)forState:UIControlStateNormal];
    }
...
//    [Util showAlertWithTitle:@"复制" message:@"是否f拷贝到剪切板" completeBlock:^(NSInteger buttonIndex) {
//
//    }];
    [_mBGView makeToast:@"日志已拷贝到剪切板" duration:1.5 position:CSToastPositionCenter];
//    HDWarnLog(tipString);
}
