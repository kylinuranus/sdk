//
//  SOSNStytleEditInfoViewController.m
//  Onstar
//
...
@interface SOSNStytleEditInfoViewController ()<UITextFieldDelegate>{
    BOOL defaultPinFlag;
}
...
@implementation SOSNStytleEditInfoViewController
...
#pragma mark UI
- (void)configUI {
    [self.nextButton setNSButtonStyle];
    switch (self.editType) {
        ...
        {
            self.title = @"车辆控制密码";
            self.tipNormalText = @"密码为6字符，可为数字或数字与字母组合，特殊字符除外\n字母区分大小写";
...
-(BOOL)checkUniqueFalse{
    if (self.editType == SOSEditUserInfoTypePhone) {
        if (!self.isMobileUnique) {
            ...
            [SOSUtil setButtonStateDisableWithButton:self.nextButton withColor:[UIColor colorWithHexString:@"#C3CEEC"]];
