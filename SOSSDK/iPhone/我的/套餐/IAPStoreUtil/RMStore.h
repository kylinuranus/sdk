...
/**(支付类型：这个枚举非RMStore所有，是客户端自己添加的，主要的目的是：区分是谁调用的是什么)*/
typedef NS_ENUM(NSInteger, RMStorePurchasePayType) {
    RMStorePurchasePayNone = 0, // 默认
    RMStorePurchasePayRearviewMirror = 1, // 后视镜
};
@interface RMStore : NSObject<SKPaymentTransactionObserver>

/* Values for HTTPMethod */
...
/** 支付类型 */
@property (nonatomic, assign) RMStorePurchasePayType purchasePayType;
