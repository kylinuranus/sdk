...
@implementation RMStore {
    ...
    NSString *_onStarTransactionIdentifier;//安吉星订单号
...
- (void)start {
    ...
    self.transactionPersistor = persistence;
    RMStoreTransactionReceiptVerifier *receiptVerifier = [[RMStoreTransactionReceiptVerifier alloc] init];
...
transactionIdentify:(NSString *)transactionIdentify
           success:(void (^)(SKPaymentTransaction *transaction))successBlock
           failure:(void (^)(SKPaymentTransaction *transaction, NSError *error))failureBlock
{
    _onStarTransactionIdentifier = transactionIdentify;
    [self addPayment:productIdentifier
                user:userIdentifier
...
- (void)didPurchaseTransaction:(SKPaymentTransaction *)transaction queue:(SKPaymentQueue*)queue
{
    ...
    {
        //将收据和onstar订单 apple订单存本地
        [[RMStore defaultStore].transactionPersistor addTransactionWith:transaction onStarTransactionIdentifier:_onStarTransactionIdentifier];
