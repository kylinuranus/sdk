...
@interface SOSQRManager () <AVCaptureMetadataOutputObjectsDelegate>
{
...
@implementation SOSQRManager


static SOSQRManager *_instance;
...
- (BOOL)checkAuthStatus	{
    if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        [Util showAlertWithTitle:nil message:@"检测不到相机设备" completeBlock:nil cancleButtonTitle:@"确定" otherButtonTitles:nil];
        return NO;
    }
    AVAuthorizationStatus authStatus = [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo];
    if(authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied){
        [Util showAlertWithTitle:nil message:@"相机权限受限，请到系统设置中开启相机权限" completeBlock:nil cancleButtonTitle:@"确定" otherButtonTitles:nil];
        return	NO;
    }
    return YES;
}
...
- (void)startScanningQRCodeWithInView:(UIView *)inView scanView:(UIView *)scanView resultCallback:(void(^)(NSArray *results))callback {
    ...
    // 5.2.设置的layer的frame
//    previewLayer.frame = inView.frame;
    NSLog(@"%@",inView);
    
    CALayer *viewLayer = [inView layer];
    [viewLayer setMasksToBounds:YES];
    
    CGRect bounds = [inView bounds];
    [previewLayer setFrame:bounds];
    NSLog(@"preview.height>>>>>%f",[previewLayer frame].size.height);
    NSLog(@"preview.width>>>>>%f",[previewLayer frame].size.width);
    
    if ([[previewLayer connection] isVideoOrientationSupported]) {
        NSLog(@"isVideoOrientationSupported=true");
...
- (void)startScanningQRCodeWithInView01:(UIView *)inView scanView:(UIView *)scanView resultCallback:(void(^)(NSArray *results))callback {
    ...
    [viewLayer setMasksToBounds:YES];
    // 5.3.将图层添加到其它图层中
    [inView.layer insertSublayer:previewLayer atIndex:0];
